...
## "Entropy Loss": Output Layer Uncertainty {.smaller .crunch-title .title-10 .math-80 .text-70 .crunch-quarto-figure}
...
![Step 2: NN starting to converge: $\Pr(Y = 9)$ high, $\Pr(Y = 3)$ medium, $\Pr(Y = d)$ low for all other $d$](images/nn_crossent_success.svg){fig-align="center"}
...
## The Problem With Entropy Loss {.smaller .crunch-title .title-10 .math-80 .text-70}

:::: {.columns}
::: {.column width="33%"}

Max entropy = max uncertainty (0-9 equally likely)
...
![Step 1: NN has no idea, guesses (via softmax) $\Pr(y = d) = 0.1$ for every $d$](images/nn_crossent.svg){fig-align="center"}

:::
::: {.column width="33%"}

Middle entropy = less uncertainty

$$
\Pr(y = 9) > \Pr(y = 3) > \Pr(y = d')
$$

![Step 2: NN starting to converge: probably $d = 9$, maybe $d = 3$, low probability on all other values](images/nn_crossent_success.svg){fig-align="center"}
...
![Step 3: NN has converged to predicting ultra-high $\Pr(y = 9 \mid X)$](images/nn_minent.svg){fig-align="center"}
...
![Step 2: NN is starting to converge to correct prediction: probably $d = 9$, maybe $d = 3$, low probability on all other values](images/nn_crossent_success.svg){fig-align="center"}
